...
void RobotContainer::ConfigDriverBindings() {
    ...
    driver.POVLeft().Debounce(200_ms).WhileTrue(AlgaeGroundCommand(&stateManager));
    driver.POVLeft().Debounce(200_ms).OnFalse(
...
void RobotContainer::ConfigMixedBindigs() {
    (driver.POVDown() && console.Button(12)).OnTrue(
            L2Command(&stateManager, &alignManager).AlongWith(leftAlignPos(&alignManager)).BeforeStarting(
                    frc2::cmd::RunOnce([this] {
                        alignManager.setHeading();
                    })));

    (driver.POVDown() && console.Button(5)).OnTrue(
            L2Command(&stateManager, &alignManager).AlongWith(rightAlignPos(&alignManager)).BeforeStarting(
                    frc2::cmd::RunOnce([this] {
                        alignManager.setHeading();
                    })));
