...
Arm::Arm() {
    ...
    armLeftMotor.configureMotionMagic(Constants::ArmCruiseVelocity,
            Constants::ArmCruiseAcceleration, 0.0_tr_per_s_cu);
    wristMotor.configureMotionMagic(Constants::WristCruiseVelocity,
            Constants::WristCruiseAcceleration, 0.0_tr_per_s_cu);
}
...
void Arm::setToAngle(units::degree_t armAngle, units::degree_t wristAngle) {
    ...
    armLeftMotor.SetControl(
            armVoltage.WithPosition(armAngle).WithEnableFOC(true));
    wristMotor.SetControl(
            wristVoltage.WithPosition(wristAngle).WithEnableFOC(true));
}
;
...
bool Arm::isArmAtPosition(units::degree_t armAngle,
    ...
    units::degree_t wristError = wristAngle
...
void Arm::Periodic() {
    ...
    frc::SmartDashboard::PutNumber("Arm/Current Wrist Angle",
            wristCurrentAngle);
}
